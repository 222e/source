...
function loadColorDefaultStyle() {
	// colors array
	var defaultColors = [
		['--color-primary-darker', [220, 0.9, 0.36]],
		['--color-primary-dark', [220, 0.9, 0.46]],
		['--color-primary', [220, 0.9, 0.56]],
		['--color-primary-light', [220, 0.9, 0.66]],
		['--color-primary-lighter', [220, 0.9, 0.76]],
		['--color-accent-darker', [349, 0.75, 0.31]],
		['--color-accent-dark', [349, 0.75, 0.41]],
		['--color-accent', [349, 0.75, 0.51]],
		['--color-accent-light', [349, 0.75, 0.61]],
		['--color-accent-lighter', [349, 0.75, 0.71]],
		['--color-black', [240, 0.08, 0.12]],
		['--color-white', [0, 0, 1]],
		['--color-success-darker', [94, 0.48, 0.36]],
		['--color-success-dark', [94, 0.48, 0.46]],
		['--color-success', [94, 0.48, 0.56]],
		['--color-success-light', [94, 0.48, 0.66]],
		['--color-success-lighter', [94, 0.48, 0.76]],
		['--color-error-darker', [349, 0.75, 0.31]],
		['--color-error-dark', [349, 0.75, 0.41]],
		['--color-error', [349, 0.75, 0.51]],
		['--color-error-light', [349, 0.75, 0.61]],
		['--color-error-lighter', [349, 0.75, 0.71]],
		['--color-warning-darker', [46, 1, 0.41]],
		['--color-warning-dark', [46, 1, 0.51]],
		['--color-warning', [46, 1, 0.61]],
		['--color-warning-light', [46, 1, 0.71]],
		['--color-warning-lighter', [46, 1, 0.81]],
		['--color-bg', [0, 0, 1]],
...
function loadTypographyStyle(typography, bool, mq) {
	...
	loadTypographyStyleTag(getTypographyStyle(headingValues, 'h1, h2, h3, h4', typography) + getTypographyStyle(bodyValues, 'body', typography));
...
function loadTypographyDefaultStyle(bool, mq) {
	...
	var bodyStyle = '--font-primary: Inter, sans-serif;--text-base-size: 1em;--text-scale-ratio: 1.2;--text-xs: calc((1em / var(--text-scale-ratio)) / var(--text-scale-ratio));--text-sm: calc(var(--text-xs) * var(--text-scale-ratio));--text-md: calc(var(--text-sm) * var(--text-scale-ratio) * var(--text-scale-ratio));--text-lg: calc(var(--text-md) * var(--text-scale-ratio));--text-xl: calc(var(--text-lg) * var(--text-scale-ratio));--text-xxl: calc(var(--text-xl) * var(--text-scale-ratio));--text-xxxl: calc(var(--text-xxl) * var(--text-scale-ratio));--text-xxxxl: calc(var(--text-xxxl) * var(--text-scale-ratio));--body-line-height: 1.4;--heading-line-height: 1.2;--font-primary-capital-letter: 1;';
	...
	loadTypographyStyleTag(bodyStyle+headingStyle+bodyResStyle);
...
function loadTypographyStyleTag(styleContent) {
	var defaultStyle = 'mark {background-color: hsla(var(--color-accent-h), var(--color-accent-s), var(--color-accent-l), 0.2);color: inherit;}.text-component {--line-height-multiplier: 1;--text-vspace-multiplier: 1;}.text-component blockquote {padding-left: 1em;border-left: 4px solid var(--color-contrast-low);}.text-component hr {background: var(--color-contrast-low);height: 1px;}.text-component figcaption {font-size: var(--text-sm);color: var(--color-contrast-medium);}';
...
function setTypographyCssVariables(headingValues, bodyValues, typography, mq) {
	...
	// set text
	var scaleRatioValueRes = getValidResponsiveValue(Object.values(typography['scaleRatio']), mq, false),
		typeScaleVaraiables = ['--text-xxxxl', '--text-xxxl', '--text-xxl', '--text-xl', '--text-lg', '--text-md', '--text-body', '--text-sm', '--text-xs'];
	for(var i = 6, j = 0; i > -3; i--, j++) {
		var typeScale = Math.round(Math.pow(scaleRatioValueRes, i)*1000)/1000+'em';
		if( j != 6) setCssVariable(typeScaleVaraiables[j], typeScale);
	}
};
...
function loadButtonsStyle(buttons) {
	...
	var btnsList = ['btn', 'btn--primary', 'btn--subtle', 'btn--accent', 'btn--disabled', 'btn--sm', 'btn--md', 'btn--lg'],
		...
		code = '';
	code = resetButtonsStyleComponents();
	// reset hover effect if needed
	var btnHover = ['btn--primary', 'btn--subtle', 'btn--accent'];
	for(var i = 0; i < btnHover.length; i++) {
		if(buttonsArray[btnHover[i]][':hover'] && buttonsArray[btnHover[i]][':hover']['appearance']) {
			if(!buttonsArray[btnHover[i]][':hover']['appearance'][0] || buttonsArray[btnHover[i]][':hover']['appearance'][0] == '') {
				code = code + getHoverFallback(buttonsArray, btnHover[i]);
			}
		} else {
			code = code + getHoverFallback(buttonsArray, btnHover[i]);
		}
	...
	for(var i = 0; i < btnsList.length; i++) {
		code = code + buttonsCodeStyle(buttonsArray[btnsList[i]], btnsList[i], statusList, statusPriorityList, false);
	}
...
function loadButtonsDefaultStyle(mq) {
	var btnsStyle = ':root {--btn-font-size: 1em;--btn-font-size-sm: 0.8em;--btn-font-size-md: 1.2em;--btn-font-size-lg: 1.4em;--btn-padding-x: var(--space-md); --btn-padding-y: var(--space-xs); -btn-radius: 0.25em;}.btn {cursor: pointer;text-decoration: none;line-height: 1;box-shadow: 0px 2px 8px  hsla(var(--color-black-h), var(--color-black-s), var(--color-black-l), 0.12), 0px 1px 1px  hsla(var(--color-black-h), var(--color-black-s), var(--color-black-l), 0.12);transition: all 0.3s ease;}.btn:hover {box-shadow: 0px 1px 2px  hsla(var(--color-black-h), var(--color-black-s), var(--color-black-l), 0.12), 0px 1px 1px  hsla(var(--color-black-h), var(--color-black-s), var(--color-black-l), 0.12);}.btn:focus {box-shadow: 0px 0px 0px 2px hsla(var(--color-black-h), var(--color-black-s), var(--color-black-l), 0.2);outline: none;}.btn:active {transform: translateY(2px);}.btn--primary {background-color: var(--color-primary);color: var(--color-white);-webkit-font-smoothing: antialiased;-moz-osx-font-smoothing: grayscale;}.btn--primary:hover {background-color: var(--color-primary-dark);}.btn--primary:focus {box-shadow: 0px 0px 0px 2px hsla(var(--color-primary-h), var(--color-primary-s), var(--color-primary-l), 0.2);}.btn--subtle {background-color: var(--color-contrast-lower);color: var(--color-contrast-higher);}.btn--accent {background-color: var(--color-accent);color: var(--color-white);-webkit-font-smoothing: antialiased;-moz-osx-font-smoothing: grayscale;}.btn--accent:hover {background-color: var(--color-accent-dark);}.btn--accent:focus {box-shadow: 0px 0px 0px 2px hsla(var(--color-accent-h), var(--color-accent-s), var(--color-accent-l), 0.2);}.btn--disabled {opacity: 0.6;cursor: not-allowed;}';
...
function buttonVariables(btnStyle) {
	...
	if(btnStyle['btn'] && btnStyle['btn']['none'] && btnStyle['btn']['none']['appearance'] && btnStyle['btn']['none']['appearance'][2]) {
...
function loadFormsStyle(forms) {
	...
	//reset focus effect if needed
	if(!formsArray['form-control'][':focus']['border'] || !formsArray['form-control'][':focus']['border'][0] || !formsArray['form-control'][':focus']['border'][0][0] || !formsArray['form-control'][':focus']['border'][0][0] == '') {
		if(formsArray['form-control']['none']['border'] && formsArray['form-control']['none']['border'][0] && formsArray['form-control']['none']['border'][0][0] && formsArray['form-control']['none']['border'][0][0] != '') {
			code = code + '.form-control:focus{'+getButtonsBorderStyle(formsArray['form-control']['none']['border'], '', false)+'}';
		}
	}
	if(!formsArray['form-control'][':focus']['shadow'] || !formsArray['form-control'][':focus']['shadow'][0] || !formsArray['form-control'][':focus']['shadow'][0][1] || !formsArray['form-control'][':focus']['shadow'][0][1] == '') {
		if(boxShadowIsDefined(formsArray['form-control']['none']['shadow'])) {
			code = code + '.form-control:focus{'+getButtonsShadowStyle(formsArray['form-control']['none']['shadow'], '', false, true)+'}';
		}
	}
	if(!formsArray['[aria-invalid]'][':focus']['shadow'] || !formsArray['[aria-invalid]'][':focus']['shadow'][0] || !formsArray['[aria-invalid]'][':focus']['shadow'][0][1] || !formsArray['[aria-invalid]'][':focus']['shadow'][0][1] == '') {
		if(boxShadowIsDefined(formsArray['[aria-invalid]']['none']['shadow'])) {
			code = code + '.form-control[aria-invalid="true"]:focus{'+getButtonsShadowStyle(formsArray['[aria-invalid]']['none']['shadow'], '', false, true)+'}';
		} else if(boxShadowIsDefined(formsArray['form-control'][':focus']['shadow'])) {
			code = code + '.form-control[aria-invalid="true"]:focus{'+getButtonsShadowStyle(formsArray['form-control'][':focus']['shadow'], '', false, true)+'}';
		} else if(boxShadowIsDefined(formsArray['form-control']['none']['shadow'])) {
			code = code + '.form-control[aria-invalid="true"]:focus{'+getButtonsShadowStyle(formsArray['form-control']['none']['shadow'], '', false, true)+'}';
		}
	}
	if(!formsArray['[disabled]']['none']['appearance'] || !formsArray['[disabled]']['none']['appearance'][4] || formsArray['[disabled]']['none']['appearance'][4] != 0 ) {
		if(formsArray['form-control']['none']['appearance'] && formsArray['form-control']['none']['appearance'][4] && formsArray['form-control']['none']['appearance'] != 0) {
			code = code + '.form-control[disabled], .form-control[readonly]{cursor: '+getCursorValue(formsArray['form-control']['none']['appearance'][4])+';}';
		}
	}
...
function formVariables(formStyle) {
	...
	if(formStyle && formStyle['none'] && formStyle['none']['appearance'] && formStyle['none']['appearance'][2]) {
		tring = string + '--form-control-radius:'+formStyle['none']['appearance'][2]+';';
	}
	return ':root{'+string+'}';
};

function loadFormsDefaultStyle(mq) {
	var formStyle = ':root {--form-control-padding-x: var(--space-sm);--form-control-padding-y: var(--space-xs);--form-control-radius: 0.25em;}.form-control {background-color: var(--color-bg);border: 2px solid var(--color-contrast-low);transition: all 0.3s ease;}.form-control:focus {background-color: var(--color-bg);border-color: var(--color-primary);box-shadow: 0px 0px 0px 3px hsla(var(--color-primary-h), var(--color-primary-s),var(--color-primary-l), 0.2);outline: none;}.form-control[aria-invalid="true"] {border-color: var(--color-error);}.form-control[aria-invalid="true"]:focus {box-shadow: 0px 0px 0px 3px hsla(var(--color-error-h), var(--color-error-s),var(--color-error-l), 0.2);}.form-control[disabled], .form-control[readonly] {cursor: not-allowed;}.form-label {font-size: var(--text-sm);}';
...
function getHoverFallback(array, btnLabel) {
	var colorVal;
	if(array[btnLabel]['none']['appearance'] && array[btnLabel]['none']['appearance'][0] && array[btnLabel]['none']['appearance'][0] != '') {
		colorVal = array[btnLabel]['none']['appearance'][0];
	} else if(array['btn'][':hover']['appearance'] && array['btn'][':hover']['appearance'][0] && array['btn'][':hover']['appearance'][0] != '') {
		colorVal = array['btn'][':hover']['appearance'][0];
	} else {
		colorVal = (array['btn']['none']['appearance'][0] && array['btn']['none']['appearance'][0] != '') ? array['btn']['none']['appearance'][0] : 'transparent';
	}
	var color = getColorValue(colorVal, true);
	return '.'+btnLabel+':hover{background-color:'+color+';}';
};
...
function buttonsCodeStyle(style, btnLabel, statusList, statusPriorityList, isDemoReset) {
	var string = '';
	if(!style) return string;
	var btnLabelmodified = btnLabel;
	if(btnLabel == '[disabled]') {
		btnLabelmodified = 'form-control[disabled], .form-control[readonly]';
	} else if(btnLabel == '[aria-invalid]') {
		btnLabelmodified = 'form-control[aria-invalid="true"]';
	} else if(btnLabel == 'legend') {
		btnLabelmodified = 'form-legend';
	}
	for(var i = 0; i < statusPriorityList.length; i++) {
		var styleLabel = (i == 0) ? '.'+btnLabelmodified : '.'+btnLabelmodified+statusPriorityList[i];
		var newCode = getButtonCode(style[statusPriorityList[i]], '', true, btnLabel, statusPriorityList[i], isDemoReset);
...
function getButtonCode(array, spacing, reset, btnLabel, btnStatus, isDemoReset) {
	var string = '';
...
function getButtonsAppearanceStyle(array, spacing, reset, btnLabel, btnStatus) {
	var string = '';
	if(array[0]) string = string + '\n' + spacing+'background-color: '+getColorValue(array[0], reset)+';';
	...
	if( reset && reset != true && btnStatus == 'none' && (btnLabel == 'btn' || btnLabel == 'form-control')) {
		...
		if(array[2] && array[2] != '') string = string + '\n' + spacing+'border-radius: '+array[2]+';';
	}
...
function getButtonsTypographyStyle(array, spacing, reset, btnLabel, btnStatus) {
	...
	if( reset && reset != true && btnStatus == 'none' && (btnLabel == 'btn' || btnLabel == 'btn--sm' || btnLabel == 'btn--md' || btnLabel == 'btn--lg' )) {
...
function getButtonsPlaceholderStyle(array, spacing, reset, btnLabel, btnStatus) {
	...
	if(reset && reset === true) {
		...
		string = string+selector+'::-webkit-input-placeholder {'+style+'\n'+spacing+'}';
		string = string+selector+'::-moz-placeholder {opacity: 1;'+style+'\n'+spacing+'}';
		string = string+selector+':-ms-input-placeholder {'+style+'\n'+spacing+'}';
		string = string+selector+':-moz-placeholder {'+style+'\n'+spacing+'}';
	} else {
		string = string + '\n\n'+spacing+'&::-webkit-input-placeholder {'+style+'\n'+spacing+'}';
		string = string + '\n\n'+spacing+'&::-moz-placeholder {'+style+'\n'+spacing+'}';
		string = string + '\n\n'+spacing+'&:-ms-input-placeholder {'+style+'\n'+spacing+'}';
		string = string + '\n\n'+spacing+'&:-moz-placeholder {'+style+'\n'+spacing+'}';
	}
	return string;
};
...
function resetButtonsStyleComponents() {
 	var style;
	style = '.btn{line-height: normal;padding: 0;border-radius: 0;font-size: inherit; box-shadow: none; cursor: auto;}';
	style = style + '.btn--primary{background-color: inherit;color: inherit;-webkit-font-smoothing: auto;-moz-osx-font-smoothing: auto;}';
	style = style + '.btn--subtle{color: inherit;background-color: inherit}';
	style = style + '.btn--accent{background-color: inherit;color: inherit;-webkit-font-smoothing: auto;-moz-osx-font-smoothing: auto;}';
	style = style + '.btn--disabled {cursor: auto;opacity: 1;}';
	syle = style + '.btn--sm, .btn--md, .btn--lg  {font-size: inherit;}';
	return style;
};

function resetFormsStyleComponents() {
 	var style = '';
	style = '.form-control { padding: 0;background-color: inherit;border: inherit;border-radius: 0;}';
	style = style + '.form-control:focus { outline-color: currentColor; border-color: inherit;box-shadow: inherit !important;}';
	style = style + '.form-control::-webkit-input-placeholder{color: inherit;}';
	style = style + '.form-control::-moz-placeholder{color: inherit;}';
	style = style + '.form-control:-ms-input-placeholder{color: inherit;}';
	style = style + '.form-control:-moz-placeholder{color: inherit;}';
	style = style + '.form-control[disabled], .form-control[readonly] {cursor: inherit;}';
	style = style + '.form-control[aria-invalid="true"] {border-color: inherit;}';
	style = style + '.form-control[aria-invalid="true"]:focus{box-shadow: inherit !important;}';
	style = style + '.form-legend {color: inherit;line-height: normal;margin-bottom: 0;}';
	style = style + '.form-legend, .form-label {font-size: inherit;}';
	return style;
};
...
(function() {
	...
	function updateLabelSizeWindow() {
		...
		if(resizing) {
			...
			demoViewport.text($(window).outerWidth()+'px x '+$(window).outerHeight()+'px'+mqLabelText).show();
